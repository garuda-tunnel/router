...
"""Thin ctypes wrapper around libnftables used by IPT server components."""
...
class SchemaValidator:
    ...
    def __init__(self):
        """Load jsonschema validator and bundled nftables schema file."""
    ...
    def validate(self, json):
        """Validate provided JSON object against bundled nftables schema."""
...
class Nftables:
    ...
    def __del__(self):
        """Release libnftables context when wrapper object is destroyed."""
        if self.__ctx is not None:
            self.nft_ctx_free(self.__ctx)
            self.__ctx = None

    def _flags_from_numeric(self, flags_dict, val):
        """Translate numeric bitmask into list of symbolic flag names."""
        names = []
    ...
    def _flags_to_numeric(self, flags_dict, values):
        """Translate symbolic flag names or ints into one numeric bitmask."""
    ...
    def cmd(self, cmdline, json_decode=True):
        ...
        if json_decode and output:
            try:
                t = json.loads(output)
                output = {}
                output["metainfo"] = t["nftables"][0]
                output["data"] = t["nftables"][1:]

            except json.JSONDecodeError:
                pass  # Output is not a valid JSON, leave it as is.
